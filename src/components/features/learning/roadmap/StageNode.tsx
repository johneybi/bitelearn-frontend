...
import type { ChapterSummaryDto, Category } from '@/api/learning/learning.types';
...
export default function StageNode({
  ...
  const emojis = categoryCode ? EMOJI_BY_CATEGORY[categoryCode] : FALLBACK_EMOJIS;
  ...
  return (
    ...
    >
      <Button
        ...
        variant="ghost"
        className={cn(
          'relative z-10 flex h-20 w-20 items-center justify-center p-0 hover:bg-transparent transition-all group disabled:opacity-100',
      ...
      >
        ...
        <div 
          className={cn(
            "absolute inset-x-[2px] bottom-0 top-11 rounded-b-[22px] shadow-md bg-gradient-to-t from-[#d9dee5] from-0% to-[#bdc6d0] to-[60%]"
          )}
        />
        ...
        <div
          className={cn(
            ...
            isLocked ? "bg-slate-100 border-white/60" : "bg-white border-white/80"
          )}
          style={{
            transformOrigin: 'top',
        ...
        <div className="relative z-20 flex flex-col items-center justify-center">
          {isCompleted ? (
            <Check size={36} strokeWidth={4} className="text-primary -mt-2.5" />
          ) : isLocked ? (
            <span className="text-[34px] -mt-3 tossface">🔒</span>
          ) : (
            <span className="text-[38px] -mt-3 tossface">
              {currentEmoji}
            </span>
          )}
        </div>
